#!/usr/bin/env bash
# Stage 3: COCO 上的剪枝模型蒸馏训练单一入口脚本（Pruned30）
# 目标：稳定冲击“核心精度损失 <= 2%”
...
CFG="${CFG:-experiments/DIST/pruned30_distill_coco.py}"
WORK_DIR="${WORK_DIR:-experiments/DIST/work_dirs/pruned30_distill_coco}"
LOG_FILE="${LOG_FILE:-experiments/DIST/logs/distill_pruned30_coco.log}"
SUMMARY_LOG="${SUMMARY_LOG:-experiments/DIST/logs/distill_pruned30_coco.summary.log}"
...
#    bash experiments/DIST/run_distill_pruned30.sh
# 2) 断点续训：
#    RESUME=1 bash experiments/DIST/run_distill_pruned30.sh
RESUME="${RESUME:-0}"
...
echo "=========================================="
echo "Stage3 Distill (Pruned30) 启动参数"
echo "CFG         : ${CFG}"
