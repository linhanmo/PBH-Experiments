...
class ViTMoEPrunable(BaseBackbone):
    ...
    def __init__(self,
        ...
        if mlp_hidden_dims is None:
            mlp_hidden_dims = [int(embed_dim * mlp_ratio)] * depth
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_hidden_dim=mlp_hidden_dims[i], qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward_features(self, x, dataset_source=None):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, indices)
            else:
                x = blk(x, indices)
